...
class ParkRepositoryFirestoreTest {
  ...
  fun getParkByPidWithValidPidReturnsPark() = runTest {
    ...
    `when`(document.getString("name")).thenReturn("Sample Park")
    `when`(document.get("location")).thenReturn(ParkLocation(0.0, 0.0, "321"))
    `when`(document.getString("imageReference")).thenReturn("parks/sample.png")
    `when`(document.getDouble("rating")).thenReturn(4.0)
    `when`(document.getLong("nbrRating")).thenReturn(2L)
    `when`(document.getLong("capacity")).thenReturn(10L)
    `when`(document.getLong("occupancy")).thenReturn(5L)
    `when`(document.get("events")).thenReturn(listOf("event1", "event2"))
