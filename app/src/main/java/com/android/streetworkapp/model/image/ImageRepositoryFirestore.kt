...
class ImageRepositoryFirestore(private val db: FirebaseFirestore, private val parkRepository: ParkRepository, private val userRepository: UserRepository): ImageRepository {
    ...
    override suspend fun uploadImage(imageB64: String, parkId: String, userId: String) {
        ...
        try {
            ...
            if (collectionId.isEmpty()) { //no imagesCollection for the park yet
                ...
                //could have some loose collections if the call fails here, can't be bothered to handle this case
    ...
    override suspend fun retrieveImages(park: Park): List<ParkImage> {
        ...
        try {
            ...
            val collection = db.collection(COLLECTION_PATH).document(park.imagesCollectionId).get().await()
            if (!collection.exists())
                throw IllegalArgumentException("The ImagesCollection linked by this park doesn't exist, the entry in the db is corrupted.")

            val parkImages = collection.get("images") as? List<*>
            ...
            return parkImages?.mapNotNull { image ->
                runCatching {
                    ...
                    val uploadDate = imageMap["uploadDate"] as? Timestamp
