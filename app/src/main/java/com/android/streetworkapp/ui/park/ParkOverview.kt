...
fun ImageTitle(
  ...
  Box(modifier = Modifier.fillMaxWidth().height(220.dp).testTag("imageTitle")) {
    ...
    Text(
        text = park?.name ?: context.getString(R.string.ParkOverviewLoading),
        color = Color.White,
        fontSize = 24.sp,
  ...
  when {
    user == null -> {
      if (context != null)
          Toast.makeText(
                  context,
                  context.getString(R.string.ParkOverviewToastRatingFailUser),
                  Toast.LENGTH_SHORT)
              .show()
    }
    park == null -> {
      if (context != null)
          Toast.makeText(
                  context,
                  context.getString(R.string.ParkOverviewToastRatingFailPark),
                  Toast.LENGTH_SHORT)
              .show()
    }
    starRating < 1 || starRating > 5 -> {
      if (context != null)
          Toast.makeText(
                  context,
                  context.getString(R.string.ParkOverviewToastRatingFailValue),
                  Toast.LENGTH_SHORT)
              .show()
    }
    parkViewModel == null -> {
      if (context != null)
          Toast.makeText(
                  context,
                  context.getString(R.string.ParkOverviewToastRatingFailViewmodel),
                  Toast.LENGTH_SHORT)
              .show()
    }
    else -> {
      ...
      if (context != null)
          Toast.makeText(
                  context,
                  context.getString(R.string.ParkOverviewToastRatingSubmitSuccessful),
                  Toast.LENGTH_SHORT)
              .show()
...
fun EventItemList(
  ...
  Column(modifier = Modifier.testTag("eventItemList")) {
    Text(
        text = context.getString(R.string.ParkOverviewEventTitle),
        fontSize = 24.sp,
        fontWeight = FontWeight.Bold,
        modifier = Modifier.padding(start = 16.dp, top = 6.dp, bottom = 2.dp))
    ...
    when (uiState) {
      ...
      is EventOverviewUiState.Empty -> {
        Box(modifier = Modifier.fillMaxSize()) {
          Text(
              text = context.getString(R.string.ParkOverviewNoEvents),
              fontSize = 16.sp,
              fontWeight = FontWeight.Light,
              modifier =
...
fun EventItem(event: Event, eventViewModel: EventViewModel, navigationActions: NavigationActions) {
  ...
  ListItem(
      ...
      supportingContent = {
        Text(
            context.getString(
                R.string.ParkOverviewEventNbrParticipant,
      ...
      overlineContent = {
        Text(
            text = dateDifference(context, event),
            fontWeight = FontWeight.Bold,
      ...
      trailingContent = {
        Button(
            ...
            contentPadding = PaddingValues(0.dp)) {
              Text(
                  context.getString(R.string.ParkOverviewEventButton),
                  modifier = Modifier.testTag("eventButtonText"))
            }
      })
  HorizontalDivider()
}
