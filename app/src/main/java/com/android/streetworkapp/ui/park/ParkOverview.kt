...
fun ParkOverviewScreen(
  ...
  Box(modifier = Modifier.padding(innerPadding).fillMaxSize().testTag("parkOverviewScreen")) {
    Column {
      ImageTitle(imageViewModel, image = null, park = currentPark.value, user = currentUser)
 ...
 */
@Composable
fun ImageTitle(imageViewModel: ImageViewModel, image: Painter?, park: Park?, user: User?) {
